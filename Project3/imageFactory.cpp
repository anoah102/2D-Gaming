...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string, SDL_Surface*>::iterator sur_it = surfaces.begin();

  while(sur_it != surfaces.end()){
      SDL_FreeSurface(sur_it->second);
      sur_it++;

  }

  std::map<std::string, SDL_Texture*>::iterator tex_it = textures.begin();

  while(tex_it != textures.end()){
    SDL_DestroyTexture(tex_it->second);
    tex_it++;
  ...
  std::map<std::string, Image*>::iterator img_it = images.begin();

  while(img_it != images.end()){
    std::cout << "deleting " << img_it->first << std::endl;
